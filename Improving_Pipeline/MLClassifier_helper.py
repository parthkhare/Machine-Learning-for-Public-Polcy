...
def generate_binary_at_k(y_scores, k):
    ...
    Note this metric is useful only for large size of data, with smaller size values len(y_scores) might bias cutoff
    Input: Column (Pnadas DF/Series) Threshold
...
def clf_loop(models_to_run, clfs, grid, X_train, X_test, y_train, y_test, training_dates, testing_dates):
    '''
    Run lopp over all defined models to run and all permutations of respective hyperparameters
        ...
        for index,clf in enumerate([clfs[x] for x in models_to_run]):
            ...
            for p in ParameterGrid(parameter_values):
                try:
                    ...
                    print(clfs_time,':Time for running',clf)
...
def best_model(model_data, model_results, time_var, pred_var, tr_date, te_date, eval_stat='f1_at_50', window='best'):
    '''
